...
def create_custom_network(n, num_influencers, influencer_degree, num_bots, bot_degree, num_sceptics, G=None, pos=None):
    if G is None:
        G = nx.random_geometric_graph(n, 0.08)
        pos = nx.get_node_attributes(G, "pos")
    else:
        if pos is None:
            # Jeśli graf podano, ale nie pozycje – wyliczamy layout
            pos = nx.get_node_attributes(G, "pos")
...
def draw_graph(G, agents, pos, step):
    """
    Rysuje stan grafu w danym kroku symulacji i zapisuje klatkę PNG do folderu 'frames/'.
    
    Kolory węzłów:
        - czerwony: zainfekowany
        - stalowy niebieski: uświadomiony (recovered)
        - niebieski: bot
        - żółty: influencer
        - szary: sceptyk
        - zielony: normalny użytkownik
    """
    ...
    # Rysowanie krawędzi z przezroczystością
    nx.draw_networkx_edges(G, pos=pos, ax=ax, alpha=0.2, width=0.5)

    # Rysowanie węzłów z kolorem, obramowaniem i przezroczystością
    ...
    # Etykieta kroku
    plt.text(0.98, 0.98, f"Krok {step}", fontsize=16,
...
# Wczytanie danych z pliku (graf nieskierowany)
...
G, agents, pos = create_custom_network(
    NUM_AGENTS, NUM_INFLUENCERS, INFLUENCER_DEGREE, NUM_BOTS, BOT_DEGREE, NUM_SCEPTICS, G, pos
)
