...
SIMULATION_STEPS = 12               # Liczba kroków czasowych
...
class Agent:
    ...
    def try_cure_others(self, neighbors):
        """Próba wyleczenia sąsiadów (działa tylko jeśli agent jest wyleczony)."""
        if hasattr(self, 'spreading_prob_aware') and np.random.rand() < self.spreading_prob_aware:
            for neighbor in neighbors:
...
def create_custom_network(n, num_influencers, influencer_degree, num_bots, bot_degree, num_sceptics):
    G = nx.random_geometric_graph(n, 0.12)
    ...
    # Wzmacnianie połączeń influencerów i botów
...
# GENEROWANIE GIFA
def create_gif(frames, output_file="Dezinformacja.gif"):
